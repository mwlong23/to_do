...
describe(List) do
  ...
  describe("#tasks") do
    it("returns an array of tasks for that list") do
      test_list = List.new({:name => "Epicodus stuff", :id => nil})
      test_list.save()
      test_task = Task.new({:description => "Learn SQL", :list_id => test_list.id(), :due_date => '2017-11-23'})
      test_task.save()
      test_task2 = Task.new({:description => "Review Ruby", :list_id => test_list.id(), :due_date => '2017-11-20'})
      test_task2.save()
  ...
  describe('#update') do
    it 'lets you update lists in the database' do
      list = List.new({name: "Homework stuff", id: nil })
  ...
  describe('#delete') do
    ...
    it "deletes a list's tasks from the database" do
      list = List.new({:name => "Epicodus stuff", :id => nil})
      list.save()
      task = Task.new({:description => "learn SQL", :list_id => list.id(), :due_date => '2017-11-23'})
      task.save()
      task2 = Task.new({:description => "Review Ruby", :list_id => list.id(), :due_date => '2017-11-23'})
      task2.save()
      list.delete()
